        ...
        <div className="space-y-3 pr-2 h-full">
            <AnimatePresence>
                {alerts.map((alert, index) => (
                    <AlertItem key={alert.id} alert={alert} index={index} onClick={() => onAlertClick && onAlertClick(alert)} />
                ))}
            </AnimatePresence>
        </div>
    );
...
const AlertItem = ({ alert, index, onClick }) => {
    ...
    const getBorderColor = () => {
        switch (alert.type) {
            case 'critical': return 'border-neon-red/40 bg-neon-red/5';
            case 'warning': return 'border-neon-yellow/40 bg-neon-yellow/5';
            case 'success': return 'border-neon-green/40 bg-neon-green/5';
    ...
    return (
      ...
      transition-all duration-300 hover:bg-slate-700/50 group
        ...
        >
            <div className="flex justify-between items-start">
                <div className="flex items-center gap-2 mb-1">
                    {getIcon()}
                    <span className={`text-xs font-mono opacity-70 group-hover:opacity-100 flex items-center gap-1`}>
